    ...
    <div class="container mx-auto py-5 max-w-7xl">
      <div class="flex justify-between items-center px-4 mt-10 mb-4">
        ...
        <div class="flex gap-4 mt-4 ml-4">
          <button @click="filter = 'all'" :class="{ 'font-bold shadow-none': filter === 'all', 'btn': true, 'btn-primary': filter !== 'all' }">All</button> 
          <button @click="filter = 'in_progress'" :class="{ 'font-bold': filter === 'in_progress', 'btn bg-yellow-500 bg-opacity-30 border border-none shadow-none': true, 'text-yellow-300': true }">In Progress</button>
          <button @click="filter = 'completed'" :class="{ 'font-bold shadow-none': filter === 'completed', 'btn bg-green-500 bg-opacity-30 border border-none shadow-none text-green-300': true }">Completed</button> 
        </div>
    </div>
      ...
      <div class="pt-40 w-full flex flex-col items-center">
        <p v-if="filteredBookings.length === 0" class="text-center text-xl cursor-pointer hover:scale-105 duration-300" @click="redirectToHome">
          <span v-if="filter === 'all'">You have not booked a service yet. Maybe try booking <span class="text-green-400">here</span>.</span>
          <span v-else>No {{ filter.replace('_', ' ') }} bookings found.</span> </p>
