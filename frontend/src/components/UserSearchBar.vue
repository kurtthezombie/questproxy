<template>
  <div class="relative w-full">
    <div
      class="flex justify-center items-center bg-gray-800 rounded-lg shadow-md border border-gray-700 focus-within:ring-2 focus-within:ring-green-500 transition-all duration-200">
      ...
      <input type="text" v-model="searchQuery" @input="handleSearchInput" @keydown.enter="handleEnterKey"
      ...
      <div class="relative">
        <button @click="toggleFilterDropdown"
          class="px-4 py-2 bg-gray-700 text-white rounded-r-lg border-l border-gray-600 focus:outline-none hover:bg-gray-600 transition-colors duration-200 text-sm"
          aria-haspopup="true" :aria-expanded="showFilterDropdown">
          {{ selectedFilter || 'Filter' }}
          <svg class="w-4 h-4 inline-block ml-1 -mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"
        ...
        <div v-if="showFilterDropdown"
          class="absolute right-0 mt-2 w-36 bg-gray-800 rounded-md shadow-lg border border-gray-700 z-30">
          <ul class="py-1">
    ...
    <div v-if="showResultsDropdown"
      class="absolute z-20 w-full mt-2 bg-gray-800 rounded-lg shadow-lg border border-gray-700 max-h-60 overflow-y-auto">
      <div v-if="error" class="p-4 text-red-400 text-sm">{{ error }}</div>
      <div v-else-if="results.length === 0 && searchQuery.length > 0 && !loading" class="p-4 text-gray-400 text-sm">No
        users found.</div>
      <div v-else-if="searchQuery.length < 2 && !loading" class="p-4 text-gray-400 text-sm">Type at least 2 characters
        to search.</div>
      <ul v-else class="divide-y divide-gray-700">
        <li v-for="user in sortedResults" :key="user.id"
          class="p-3 hover:bg-gray-700 cursor-pointer transition-colors duration-100" @click="selectUser(user)">
          <div class="flex items-center justify-between">
            <div class="flex items-center">
              <div
                class="w-8 h-8 rounded-full bg-green-500 flex items-center justify-center text-white text-sm font-semibold mr-3">
